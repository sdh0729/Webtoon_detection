...
transform = transforms.Compose([
    transforms.Resize((224, 224)),# vgg 이미지 크기
    transforms.ToTensor(),
...
class BinaryImageClassifier(nn.Module):
    def __init__(self):
        ...
        resnext = models.resnext50_32x4d(weights='ResNeXt50_32X4D_Weights.DEFAULT')  # ResNeXt 모델 로드
        self.features = nn.Sequential(*list(resnext.children())[:-1])  # 마지막 FC 레이어 제외한 모든 레이어 사용
        self.classifier = nn.Sequential(
            nn.Linear(2048, 128),  # ResNeXt의 출력 크기를 입력으로 받는 FC 레이어
            nn.ReLU(),
