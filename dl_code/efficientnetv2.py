...
transform = transforms.Compose([
    transforms.Resize((260, 260)),#eff v2-s
    transforms.ToTensor(),
    transforms.Normalize(mean=[0.678, 0.608, 0.622], std=[0.238, 0.247, 0.242]),
    #EfficientNetV2-S: Default input size is 260x260.
    #EfficientNetV2-M: Default input size is 300x300.
    #EfficientNetV2-L: Default input size is 380x380.
    #EfficientNetV2-XL: Default input size is 456x456.
])
...
class BinaryImageClassifier(nn.Module):
    def __init__(self):
        ...
        efficientnet = timm.create_model('tf_efficientnetv2_s', pretrained=True)  # Use EfficientNetV2 as backbone
        num_ftrs = efficientnet.classifier.in_features
        efficientnet.classifier = nn.Sequential(
            nn.Linear(num_ftrs, 128),
            nn.ReLU(),
            nn.Linear(128, 2)  # Binary classifier output is 2
        )
