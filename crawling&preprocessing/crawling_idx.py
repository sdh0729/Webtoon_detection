...
def get_comic_date(en, gen, name):
    date_url = "https://newtoki.help/웹툰/" + en + "/작품/" + gen + "?sst=wr_datetime"
...
def comic_download(c, r, num, title):  # c는 코믹id, r은 회차 id, num은 회차 title은 제목
    ...
    save_path = ttitle + '/' + str(num) + '화/Toon_%s'
...
if __name__ == "__main__":
    ...
    print("작품의 첫 자음을 입력해 주세요.")
    print("예 : 마음의 소리 >> ㅁ 입력")
    ge = str(input("주의 : 숫자는 0-9 , 영어는 A-Z 로 입력할 것. : "))
    ...
    try:
        if not (os.path.isdir(ttitle)):
            os.makedirs(os.path.join(ttitle))
    except OSError as e:
        if e.errno != errno.EEXIST:
            print("Failed to create directory!!!py!!")
            raise

    for i in range(len(r_id)):
        os.makedirs(os.path.join(ttitle + '/' + str(i) + '화'))
